...
import type { DataSource, CreateDataSourceData } from '../types/brand';
...
export const BrandDataSources: React.FC<BrandDataSourcesProps> = ({ 
  ...
  const fetchDataSources = async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const response = await apiService.getDataSources(brandId);
      // Ensure we have an array of data sources
      const dataSourcesArray = response?.results || [];
  ...
  const handleAddSource = async () => {
    if (!brandId || !formData.source_name || !formData.platform_type) {
      setError(i18n.language === 'vi' ? 'Vui lòng điền tên nguồn và loại nguồn' : 'Please fill in source name and type');
      return;
    }
  ...
  const handleFormChange = (field: keyof DataSourceFormData, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
    ...
    <div className="brand-data-sources-wrapper">
        ...
        <div className="section-header-info">
          ...
          <div>
            ...
            <p className="section-description">
              {i18n.language === 'vi' 
                ? `Cấu hình các nguồn dữ liệu mà AI sẽ crawl cho ${brandName}`
      ...
      {!brandId ? (
        <div className="empty-state">
          <IoWarning className="empty-icon" />
          <h3>{i18n.language === 'vi' ? 'Chưa chọn brand' : 'No Brand Selected'}</h3>
          <p>
            {i18n.language === 'vi' 
              ? 'Vui lòng chọn một brand để quản lý nguồn dữ liệu'
              : 'Please select a brand to manage data sources'}
          </p>
        </div>
      ) : showForm ? (
        <div className="add-source-form">
          ...
          <div className="form-actions">
            <button 
              ...
              disabled={isSubmitting || !formData.source_name || !formData.platform_type}
            >
      ...
      {dataSources.length === 0 && !showForm ? (
        <div className="empty-state">
          ...
          {brandId && (
            ...
            >
              <IoAddCircle className="button-icon" />
              {i18n.language === 'vi' ? 'Thêm nguồn đầu tiên' : 'Add First Source'}
            </button>
          )}
        </div>
      ) : (
